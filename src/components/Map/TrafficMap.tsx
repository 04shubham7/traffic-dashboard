...
const TrafficMap: React.FC<TrafficMapProps> = ({ onJunctionSelect, selectedJunction }) => {
  ...
  const initializeMap = useCallback(() => {
    ...
    try {
      ...
      // Reset container styles that might interfere with Leaflet
      ...
      // Create map with more defensive options
  ...
  useEffect(() => {
    ...
    const loadLeaflet = () => {
      ...
      if (window.L && mapContainerRef.current && isComponentMounted) {
        initializeMap();
        return;
      }
        ...
        script.onload = () => {
          if (isComponentMounted) {
            timeoutId = setTimeout(() => {
              if (mapContainerRef.current && isComponentMounted) {
                initializeMap();
              }
            }, 100);
          }
    ...
    <div className="traffic-map">
      <div className="map-controls">
        <div className="layer-controls">
          ...
          <button
            className={`layer-btn ${mapLayer === 'congestion' ? 'active' : ''}`}
            onClick={() => setMapLayer('congestion')}
          >
            Congestion
          </button>
          <button
            className={`layer-btn ${mapLayer === 'queue' ? 'active' : ''}`}
            onClick={() => setMapLayer('queue')}
          >
            Queue Length
          </button>
          <button
            className={`layer-btn ${mapLayer === 'speed' ? 'active' : ''}`}
            onClick={() => setMapLayer('speed')}
          >
            Speed
          </button>
          <button
            className={`layer-btn ${mapLayer === 'incidents' ? 'active' : ''}`}
